...
export class ManagedSession {
  ...
  // NOTE: there is deliberately NO log-path allowlist on this class. There
  // used to be one (_allowedLogPaths/allowLogPaths/isLogPathAllowed), seeded
  // by nobody, while GET /api/logs seeded a second, token-keyed one inside
  // buildRoutes -- two structures answering one privileged-read question,
  // with different keys, different lifetimes and only one of them ever
  // pruned. The routes-layer allowlist is the single source of truth;
  // buildRoutes hands its own `isLogPathAllowed(token, path)` back to
