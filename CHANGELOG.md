...
* Upgrade note : the tunnel-ownership marker file's format changed to support the boot check above, so a marker written by a pre-1.26.0 build is refused for both adoption and reaping. If a tunnel from an older build is still running for a given VPN config when you upgrade, it is left running (orphaned) rather than adopted or killed — this costs one leaked `wireproxy` process per config, at most once, until the machine restarts or you stop it manually. **If `vpn.socksPort` is pinned to a port that old tunnel still holds, the new build refuses to start the tunnel** rather than silently sharing the port (that silent sharing was the security gap this release closes) — you'll see an error naming the port. See the README's VPN section ("Upgrading from before 1.26.0") for the recovery step.
