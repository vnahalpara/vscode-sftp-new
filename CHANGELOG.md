...
* New Feature : an optional root-credential lane — a profile carrying both `root_user` and `root_password` runs privileged Services/Web server commands over a second SSH connection authenticated as those credentials instead of `sudo -n` under the profile's own user. On a hop/bastion profile the credentials apply to the innermost hop (the real destination), never the jump host.
* Note : privileged commands (`systemctl`, `nginx`/`apache2ctl`/`httpd`, `openssl`) require either passwordless sudo for the profile's user or the root-credential lane above; a failed action surfaces a hint naming the account/host instead of failing silently. A narrow, per-command sudoers rule (e.g. `NOPASSWD: /bin/systemctl restart nginx`) will not match what's actually run, since every command carries a `--` end-of-options guard and a fully-qualified unit name — see the README for details.
